...
export default async function ReportsPage(){await requireSession();return <PageShell eyebrow="Reports" title="Reports dashboard." intro="A single place for sales, employees, customers, appointments, inventory, messaging, and payroll reports." wide><div className="grid gap-5 lg:grid-cols-2">{groups.map(([title,items])=><section key={title as string} className="rounded-xl border border-border bg-surface p-6"><h2 className="font-serif text-2xl">{title as string}</h2><div className="mt-4 grid gap-2 sm:grid-cols-2">{(items as string[]).map(item=><button key={item} className="rounded-md border border-border bg-white px-3 py-2 text-left text-sm hover:bg-surface-elevated">{item}</button>)}</div></section>)}</div></PageShell>}
